...
chrome.storage.local.get({ apiUrl: "http://127.0.0.1:8000" }, ({ apiUrl }) => { apiInput.value = apiUrl; });

function detectPlatform(hostname) {
  const rules = [["upwork", "Upwork"], ["freelancer", "Freelancer"], ["fiverr", "Fiverr"], ["contra", "Contra"], ["peopleperhour", "PeoplePerHour"], ["workana", "Workana"], ["zbj", "猪八戒"], ["epwk", "一品威客"]];
...
capture.addEventListener("click", async () => {
  ...
  try {
    ...
    const response = await fetch(`${apiUrl}/jobs/dom-capture`, {
      ...
      body: JSON.stringify({ platform: detectPlatform(new URL(tab.url).hostname), jobs: [result] })
    });
    if (!response.ok) throw new Error((await response.json().catch(() => ({ detail: response.statusText }))).detail);
    status.textContent = "已导入本机雷达。回到网页点击“重新评分”查看。";
  } catch (error) {
